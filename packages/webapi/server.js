// Import statements - added new imports for RAG functionality
...
const endpointUrl = new URL(process.env.AZURE_INFERENCE_SDK_ENDPOINT);
const baseEndpoint = `${endpointUrl.protocol}//${endpointUrl.host}`;
const deploymentName = endpointUrl.pathname.split("/")[3]; // Extract deployment name from path
...
// PDF processing variables and functions
let pdfText = null; 
let pdfChunks = []; 
...
// Modified chat endpoint with RAG functionality
app.post("/chat", async (req, res) => {
  const userMessage = req.body.message;
  const useRAG = req.body.useRAG === undefined ? true : req.body.useRAG; 
  const sessionId = req.body.sessionId || "default"; // Use sessionId from request or default to "default"
  let messages = [];
  let sources = [];
...
app.listen(PORT, () => {
  ...
  console.log("Environment check:", {
    ...
    hasKey: !!process.env.AZURE_INFERENCE_SDK_KEY,
  });
});